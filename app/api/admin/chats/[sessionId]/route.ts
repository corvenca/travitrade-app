...
export async function GET(request: Request, { params }: { params: Promise<{ sessionId: string }> }) {
  try {
    ...
    const result = await pool.query('SELECT * FROM chat_sessions WHERE session_id = $1 ORDER BY created_at ASC', [sessionId])
...
export async function PUT(request: Request, { params }: { params: Promise<{ sessionId: string }> }) {
  try {
    ...
    await pool.query('UPDATE chat_sessions SET status = $1 WHERE session_id = $2', [status, sessionId])
